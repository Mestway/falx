...
from falx.synthesizer import utils
...
def backward_eval(node, out_df, is_outer_most=True):
	...
	Args:
		node: an ast node (generated from to_dict() method from operators in table_lang)
...
def backward_eval_one_step(op, out_df, is_outer_most=False):
	...
	cols = out_df.columns
	schema = utils.extract_table_schema(out_df)
...
if __name__ == '__main__':
	...
	for df in backward_eval_one_step("gather", out_df, is_outer_most=True):
		print('-')
		print(df)
		print(check_table_inclusion(df.to_dict(orient="records"), inputs[0], wild_card=utils.UNKNOWN))
